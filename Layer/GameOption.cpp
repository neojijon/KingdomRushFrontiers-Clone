...
void GameOption::initOption()
{
	...
	option->setPosition(Vec2(Director::getInstance()->getWinSize().width/2,Director::getInstance()->getWinSize().height/2+20));
	addChild(option);
}
...
void GameOption::initButton()
{
	...
	close->setPosition(Vec2(337,156));
	...
	effect->setPosition(Vec2(effect_bak->getContentSize().width/2,effect_bak->getContentSize().height/2));
	effect->setTag(2);
	if(UserDefault::getInstance()->getIntegerForKey("backeffect",1)==0){
		addOff(effect);	
	}
	effect_bak->addChild(effect);

	auto vibration = Sprite::createWithSpriteFrameName("options_optButtons_0005.png");
	vibration->setPosition(Vec2(vibration_bak->getContentSize().width/2,vibration_bak->getContentSize().height/2));
	vibration->setTag(3);
