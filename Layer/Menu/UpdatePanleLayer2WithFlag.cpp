...
bool UpdatePanleLayer2WithFlag::initWithIcons(int icon1,int icon2,int money1,int money2)
{
	...
	updateIcon1->setPosition(Vec2(planesprite->getContentSize().width/4 + 20,planesprite->getContentSize().height/4+30));
	updateIcon1->setVisible(false);
	updateIcon1->setTag(1);
	addChild(updateIcon1);

	updateIcon2 = BaseBuildIcon::createIcon(icon2,money2);
	updateIcon2->setPosition(Vec2(-planesprite->getContentSize().width/4 - 20,planesprite->getContentSize().height/4+30));
	...
	lock1->setPosition(Vec2(0,planesprite->getContentSize().height/2));
	addChild(lock1);

	lock2 = Sprite::createWithSpriteFrameName("main_icons_0014.png");
	lock2->setPosition(Vec2(0,planesprite->getContentSize().height/2));
	addChild(lock2);
	lock1->setVisible(false);
	lock2->setVisible(false);

	flag = Sprite::createWithSpriteFrameName("sub_icons_0001.png");
	flag->setPosition(Vec2(planesprite->getContentSize().width/4 + 15 ,-planesprite->getContentSize().height/3 + 5));
