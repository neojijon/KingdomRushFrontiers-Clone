...
bool UpdatePanleLayer2::initWithIcons(int icon1,int icon2,int money1,int money2)
{
	...
	lock1->setPosition(Vec2(planesprite->getContentSize().width/4 + 20,planesprite->getContentSize().height/4+30));
	addChild(lock1);

	lock2 = Sprite::createWithSpriteFrameName("main_icons_0014.png");
	lock2->setPosition(Vec2(-planesprite->getContentSize().width/4 - 20,planesprite->getContentSize().height/4+30));
	addChild(lock2);
