...
bool Victory::initVictory(int star,int gem)
{
	...
	auto one = Sprite::createWithSpriteFrameName(cocos2d::StringUtils::format("win_Gnome_Numbers_000%d.png",gem%10));
	auto ten = Sprite::createWithSpriteFrameName(cocos2d::StringUtils::format("win_Gnome_Numbers_000%d.png",(gem/10)%10));
	ten->setPosition(Vec2(160,60));
	one->setPosition(Vec2(180,50));
	...
	{
		frame = SpriteFrameCache::getInstance()->getSpriteFrameByName(cocos2d::StringUtils::format("victoryStars_star_00%02d.png",len));
		if(frame!=nullptr)
...
void Victory::showGems()
{
	...
	win_Gnome->runAction(MoveBy::create(1.0f,Vec2(300,0)));
}
...
void Victory::CItemAnimation()
{
	...
	auto move = MoveTo::create(0.5f,Vec2( position.x,position.y-240));
	ContinueMenuItem->runAction(move);
}
void Victory::RItemAnimation()
{
	RestartMenuItem->setVisible(true);
	auto move = MoveTo::create(0.5f,Vec2( position.x,position.y-335));
...
void Victory::showStar(int star)
{
	...
	case(1):{
		...
		star->setPosition(Vec2(position.x,position.y-100));
		star->setScale(0.0f);
		addChild(star,3);
	...
	case(3):{
		auto star1 = Sprite::createWithSpriteFrameName("victoryStars_star_0001.png");
		star1->setPosition(Vec2(position.x - 50,position.y-100));
		star1->setScale(0.0f);
		addChild(star1,3);
		star1->runAction(ScaleTo::create(0.5f,1,1));
		auto star2 = Sprite::createWithSpriteFrameName("victoryStars_star_0001.png");
		star2->setPosition(Vec2(position.x ,position.y-100));
		star2->setScale(0.0f);
		addChild(star2,3);
		star2->runAction(Sequence::create(DelayTime::create(0.5f),ScaleTo::create(0.5f,1,1),NULL));
		auto star3 = Sprite::createWithSpriteFrameName("victoryStars_star_0001.png");
		star3->setPosition(Vec2(position.x + 50,position.y-100));
		star3->setScale(0.0f);
		addChild(star3,3);
