	...
	if(couldShit && monsterCount >= 3){
		...
		if(nearestMonster!=NULL && nearestMonster->getCurrHp() > 0 )
		{
			...
			if(nearestMonster->baseSprite->getPosition().x - (this->getParent()->getPosition()+this->getPosition()).x>0){
				...
				currBullet->setPosition(Vec2(currBullet->getPosition().x-30,currBullet->getPosition().y+30));
			}else{
			...
			if(farestMonster1!=NULL && farestMonster1->getCurrHp()>0){
				Vec2 shootVector = farestMonster1->baseSprite->getPosition() - (this->getParent()->getPosition() + this->getPosition());
				if(shootVector.x>0){
					baseSprite->setFlippedX(false);
					shit->setFlippedX(false);
				}
...
void Mecha::addShit()
{
	...
	shit->setPosition(Vec2(this->getPosition().x,this->getPosition().y - baseSprite->getContentSize().height/2));
...
void Mecha::runToLocation(Vec2 point)
{
	stopAllActions();
